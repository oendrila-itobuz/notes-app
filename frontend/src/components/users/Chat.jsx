...
export default function Chat() {
  ...
  // Fetch chat messages when an admin is selected
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await userInstance.post("http://localhost:8000/chat/sendChat", {
        senderId: userId,
      ...
      if (res.data.success) {
        setMessages([...messages, { message: newMessage, userId_Sender: userId }]);
        setNewMessage("");
      }
    } catch (error) {
